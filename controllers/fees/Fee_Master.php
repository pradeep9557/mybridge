...
class fee_master extends CI_Controller {
    ...
    function all_cfp($filter_data = array()) {
        ...
        $data['cfp'] = $this->fee_collect_model->get_all_course_fee_plan($filter_data);
    ...
    function save_cfp() {
        ...
        if ($FormData != NULL) {
            if ($FormData["save_scfp"] == "Save") {
                $inserted = $this->fee_collect_model->insert_cpf($FormData);
    ...
    function index($Stu_ID = 0, $CourseID = 0, $FeeTypeID = 0, $error = '', $_err_codes = '') {
        global $data;
        $data = $this->util_model->set_error($data, $error, $_err_codes);
        $data['EnrollNo'] = "";
        $data['Fee_Plan'] = array();
        ...
        if ($Stu_ID == 0 && $CourseID == 0 && $FeeTypeID == 0) {
...
// to get fee plan has already set or not
        if ($Stu_ID != 0) {
            ...
            if (empty($s_courses))
                redirect(base_url() . "fees/Fee_Master/index/0/0/1/NoCourseFound" . ERR_DELIMETER);


            if ($CourseID != 0) {
                if (!empty($s_courses)) {
                    foreach ($s_courses as $each_c_details) {
                        if ($each_c_details['admCourseID'] == $CourseID) {
                            $data['due_day'] = $each_c_details['due_day'];
                        }
                    }
                }
//                                                         $this->util_model->printr($s_courses);
...
//                                          die();
        ...
        $data['FeeTypeID'];
        $help_data = $this->set_fee_form_data_for_help($Stu_ID, $CourseID, $data['FeeTypeID']);
        $data['Form_Field_Value'] = $help_data;
        //$this->util_model->printr($data['Form_Field_Value']);
        $data['Fully_Paid'] = FALSE;

        if ($this->fee_collect_model->check_fully_paid($Stu_ID, $CourseID)) {
            $data['Fully_Paid'] = TRUE;
        }

        if (!file_exists(APPPATH . '/views/Fee_collect/Fees_Collect_Form.php')) {
            show_404();
        }
        ...
        $this->load->view("Fee_collect/Fees_Collect_Form.php", $data);
        $this->load->view("Fee_collect/fee_collect_js.php", $data);
        $this->load->view('templates/footer.php');
        // fee history option

        $this->load->view("Fee_collect/scfp/fee_history_icon.php", $data);
    }
    ...
    function sscfp($Stu_ID = 0, $CourseID = 0, $error = '', $_err_codes = '') {
        global $data;
        $data = $this->util_model->set_error($data, $error, $_err_codes);
        if ($Stu_ID == 0 && $CourseID == 0) {
...
//                                                  redirect(base_url() . "fees/Fee_Master/index/0/0/1/RecNotFound" . ERR_DELIMETER);
        ...
        // end of filter
        // fetching previouse course fee plan
        $data['this_cfp'] = $this->fee_collect_model->get_all_course_fee_plan($filter_data);
        unset($filter_data);
        // end of fetching
        // exiting fee plan
        ...
        $this->load->view('templates/header.php', $data);
        $this->load->view('templates/common_window_pop_up.php', $data);
        $this->load->view("Fee_collect/scfp/scfp.php", $data);
        $this->load->view('templates/footer.php');
    }
    ...
    function show_stu_fee_plan($Stu_ID = '', $CourseID = '') {
        ...
        $data['stu_exiting_fee_plan'] = $this->fee_collect_model->get_stu_fee_plan($sfp_data);
        unset($sfp_data);
        return $this->load->view("Fee_collect/scfp/v_stu_fee_plan.php", $data, TRUE);
    }
    ...
    function stu_common_fee_history($Stu_ID = '', $CourseID = '') {
        ...
        $data['last_fee_record'] = $this->show_fees_record($fee_record_filter_data);
        $this->load->view("Fee_collect/common_fee_history_panel.php", $data);
    }
    ...
    function save_sscfp() {
        ...
        $CourseID = isset($FormData['CourseID']) ? $FormData['CourseID'][0] : 0; // CourseID to return on same form 
        $path = "fees/Fee_Master/sscfp/" . $Stu_ID . "/" . $CourseID . "/";
        ...
        if ($FormData != NULL) {
            if ($FormData["save_scfp"] == "Save") {
                $inserted = $this->fee_collect_model->insert_scpf($FormData);
                redirect(base_url() . $path . ($inserted['succ'] ? "0/" . $inserted['_err_codes'] : "1/" . $inserted['_err_codes']));
            }
        }
        echo "Bad required ";
    ...
    public function fee_collect() {
        ...
        $path = "fees/Fee_Master/index/" . $Stu_ID . "/" . $CourseID . "/";
...
//                            }
        ...
        if ($FormData != NULL) {
            if ($FormData["Fee_Collect"] == "Save") {
                $inserted = $this->fee_collect_model->fee_insert();
    ...
    function fee_update() {
        ...
        if ($FormData != NULL) {
            if ($FormData["Fee_Collect"] == "Update") {
                $inserted = $this->fee_collect_model->fee_update();
                //die($this->util_model->printr($inserted));
                if ($inserted['succ']) {
                    redirect(base_url() . $path . "0/" . $inserted['_err_codes'] . "#last_fee_record");
                    // passing error alert 0 means no error, 1 means Error
    ...
    function Print_Fee($ReceiptNo) {
        ...
        $data['result'] = $this->fee_collect_model->data_to_print_fee($ReceiptNo);
...
//                            }
//                            $model_result = $this->fee_collect_model->save_update_fee_plan($_POST);
//                            if ($model_result) {
...
//                                                        $data['error'] = FALSE;
//                            }
//                            $data['Fee_Plan_Detail$Fee_Plan_IDs'] = $this->fee_collect_model->Get_Fee_Plan_Details_via_ID($Fee_Plan_ID);
...
//              }
    ...
    public function set_fee_form_data_for_help($Stu_ID, $CourseID, $FeeTypeID = '') {
        global $data;
        $form_fields = array("BranchCode", "StudentName","Pro_pic", "EnrollNo", "CourseCode", "FatherName",
        ...
        $form_fields_value['Total_Paid_Amt'] = $this->fee_collect_model->total_installments_amount($Stu_ID, $CourseID);
        ...
        // to get the exiting fee plan
        $all_fee_plan = $this->fee_collect_model->get_stu_fee_plan($sfp_data);
        unset($sfp_data);

        //$all_fee_plan = $this->fee_collect_model->All_fee_plan_of_student($student_scnb_details[0]->EnrollNo, $student_scnb_details[0]->CourseCode);
        ...
        $last_record = $this->get_last_fee_record($Stu_ID, $CourseID, $FeeTypeID);
        ...
        $bal_details = $this->fee_collect_model->get_balance_details($filter_data);
        ...
        $form_fields_value['NoOfInstallment'] = $this->fee_collect_model->get_no_of_installment($Stu_ID, $CourseID, $FeeTypeID);
        return $form_fields_value;
    }
    ...
    public function get_last_fee_record($Stu_ID, $CourseID, $FeeTypeID = "") {
        return $this->fee_collect_model->last_fee_record($Stu_ID, $CourseID, $FeeTypeID);
    }

    public function show_fees_record($filter_data) {
        $data['fees_record'] = $this->fee_collect_model->get_fee_records($filter_data);
        ...
        return $this->load->view("Fee_collect/fee_record.php", $data, TRUE);
        // $this->load->view('templates/footer.php');
    }
    ...
    public function Fee_collect_save_update() {
        ...
        if (isset($_REQUEST['Fee_Collect']) && $_REQUEST['Fee_Collect'] == "Save") {
            $saved_or_updated = $this->fee_collect_model->fee_insert();
            $path = "fees/Fee_Master/index/";
            ...
            // print_r($_POST);
            $saved_or_updated[0] = $this->fee_collect_model->Fee_collect_save_update($_POST);
            $path = $saved_or_updated[0] ? "Fee_Master/Fee_Edit/0/1" : "Fee_Master/Fee_Edit/1/1";
            redirect(base_url() . $path . "/" . $_REQUEST['ID']);
        } else {
            $path = "Fee_Master/index/1/1/" . $_REQUEST['ID'];
        }
    ...
    public function Del_Fee_plan($ID, $EnrollNo, $CourseCode) {
        $path = "";
        if ($this->fee_collect_model->Del_Fee_plan($ID)) {
            $path = "Fee_Master/set_fee_plan/0/1/$EnrollNo/" . $this->util_model->url_encode($CourseCode);
            // passing error alert 0 means no error, 1 means Error
        } else {
            $path = "Fee_Master/set_fee_plan/1/1/$EnrollNo/" . $this->util_model->url_encode($CourseCode);
            // passing error alert 0 means no error, 1 means Error
        }
        redirect(base_url() . $path);
    }
    ...
    public function Del_Fee_record($ID, $EnrollNo) {
        $path = "";
        if ($this->fee_collect_model->Del_Fee_record($ID)) {
            $path = "Fee_Master/index/0/1/$EnrollNo";
            // passing error alert 0 means no error, 1 means Error
        } else {
            $path = "Fee_Master/index/1/1/$EnrollNo";
            // passing error alert 0 means no error, 1 means Error
        }
    ...
    function all_fee_records() {
        ...
        $this->load->view('templates/header.php', $data);
        $this->load->view('templates/common_window_pop_up.php', $data);
        $this->load->view("Fee_collect/all_fee_record.php", $data);
        $this->load->view("Fee_collect/fee_collect_js.php", $data);
        $this->load->view('templates/footer.php');
        // fee history option
    }
