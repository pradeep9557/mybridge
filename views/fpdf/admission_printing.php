...
$Greating_Message = "Congratulation !! You have joined IBMS Computer Education Center.";
require('fpdf.php');
$pdf = new FPDF();
for($i=0;$i<2;$i++){
$pdf->AddPage();
$pdf->border(0,0,0);
$pdf->Header("Admission Form",$copy[$i]);
...
if(file_exists(STU_UPLOAD_PATH.$basic_details->Sign)){
 $pdf->Image(base_url().STU_UPLOAD_PATH.$basic_details->Sign,$pdf->GetX()-50,$pdf->GetY()+52,50,18,'GIF');
    ...
    $pdf->MultiCell(120,5,"Note: After Due Date(".$scnb_details->Due_Date.") of Every Month, "
            . "You will be liable to pay fine 5 Rs/- per Day.",1,'L');
    ...
    $pdf->MultiCell(180,5,"2. Your Registration will be cancelled, If you will be left or absent more than 15 days.",0,'L');
