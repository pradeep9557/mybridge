<div class="panel panel-primary">
    ...
    <div class="panel-body " id="global_task_search">
        <div class="col-lg-12">
            ...
            <?php
            echo form_open("", "id='dailyTaskForm'");
            ?> 
            <div class="row" id="global_adv_adm_search" > 
                <div class="row bottom_gap">
                    <div class="col-lg-12">
                        ...
                        <div class="col-lg-4">
                            <?php
                            echo form_dropdown("sub_task_id", $sub_task_type_list, 0, "class='chosen-select form-control chosen-select'");
                            ?>
                        </div>
                    </div>
                </div> 
                ...
                <div class="row bottom_gap">
                    <div class="col-lg-12">
                        ...
                        <div class="col-lg-4 col-md-4 col-sm-6">
                            <?php
                            echo form_dropdown("Add_User", $users_list, $selUser, "class='form-control chosen-select'");
                            ?>
                        </div>
                        <div class="col-lg-2 col-md-2 col-sm-6">
                            Approved Status
                        ...
                        <div class="col-lg-4 col-md-4 col-sm-6">
                            <?php
                            echo form_dropdown("approved", $approved_flags, 0, "class='form-control chosen-select'");
                            ?>
                        </div>
                    </div>
                </div>
                ...
                <div class="row bottom_gap">
                    <div class="col-lg-12  show_date">
                        <div class="col-lg-2 col-md-2 col-sm-4 padding_top_label">Start Date</div>
                        <div class="col-lg-4 col-md-4 col-sm-8">
                            <div class='input-group date bdatetimepicker' >
                                <input type='text' class="form-control str_date_time" name="str_date_time" value="<?php echo (isset($task_data['start_date']) && $task_data['start_date'] != "") ? date(DTF, strtotime($task_data['start_date'])) : date(DTF, strtotime("-30 days")) ?>"/>
                        ...
                        <div class="col-lg-4 col-md-4 col-sm-8">
                            <div class='input-group date bdatetimepicker' >
                                <input type='text' class="form-control end_date_time" name="end_date_time" value="<?php echo (isset($task_data['end_date']) && $task_data['end_date'] != "") ? date(DTF, strtotime($task_data['end_date'])) : date(DTF) ?>"/>
                                <span class="input-group-addon">
                                    <span class="glyphicon glyphicon-calendar"></span>
                                </span>
                            </div>
        ...
        <div class="col-md-12 groupAction text-center">
            <button class="btn btn-success" onclick="bulkApprove(1)">Approve Selected</button>
            <button class="btn btn-danger" onclick="bulkApprove(0)">Reject Selected</button>
        </div>
...
<script>
                ...
                function bulkApprove(action) {
                    if (confirm('Sure to ' + action ? 'Approve All' : 'Reject All')) {
                        $.ajax({
                            type: "POST",
                            url: "<?php echo base_url(); ?>tms/daily_task/approveDailyTaskEntry",
                            data: $('.check_box:checked').serialize() + "&_action=" + action,
                            dataType: "json",
                            success: function (search_data) {
                                if (search_data.succ) {
                                    swal("Done", search_data._err_codes, "success", 2000, false);
                                    search_task_data();
                                    $(".groupAction").hide();
                                } else {
                                    swal(">__< Error", search_data._err_codes, "error");
                                }
                            }
                        });
                    }
                ...
                function search_task_data() {
                    ...
                    $.ajax({
                        type: "POST",
                        url: "<?php echo base_url(); ?>tms/daily_task/get_dailyTask_result",
                        data: $("#dailyTaskForm").serialize(),
                        dataType: "json",
                        success: function (search_data) {
                ...
                function pre_list() {
                    var page = $("#dailyTaskForm").find("input[name=page]").val();
                    if (page > 0) {
                        page = parseInt(page) - 1;
                        $("#dailyTaskForm").find("input[name=page]").val(page);
                    }
                    search_task_data();
                }
                function next_list() {
                ...
                function export_csv() {
                    $("#dailyTaskForm").attr("action", "<?php echo base_url(); ?>reports/csv_exporter/export_all_fee_records");
                    $("#dailyTaskForm").submit();
                }
                function export_xls() {
                    preloader.on();
                    $.ajax({
                        type: "POST",
                        url: "<?php echo base_url(); ?>reports/xls_exporter/export_dailyTask",
                        data: $("#dailyTaskForm").serialize(),
                        dataType: "json",
                ...
                function approve_it(comment_id, approval_status) {
                    if (confirm("Sure?")) {
                        preloader.on();
                        $.ajax({
                            type: "POST",
                            url: "<?php echo base_url(); ?>tms/daily_task/manage_comment_approval",
                            data: "comment_id=" + comment_id + "&approved=" + approval_status,
                            dataType: "json",
                            success: function (result) {
                                if (result.succ) {
                ...
                function checkAll(that) {
                    $(".check_box").prop('checked', $(that).prop('checked'));
