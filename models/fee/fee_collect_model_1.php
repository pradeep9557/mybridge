 ...
 * @author Mr. Anup
 */
class fee_collect_model_1 extends CI_Model {
...
//              }
              ...
              public function is_record_exits_in_fee_plan_table($Stu_ID) {
                            $this->db->select('*')->from(DB_PREFIX . 'fee_individual_plan')->where(array("Stu_ID" => $Stu_ID));
                            $result = $this->db->get();
              ...
              public function get_stu_fee_plan($filter_data) {
                            ...
                            $this->db->select("sfp.*,cm.CourseCode,ftype.FeeType_Code")->from(DB_PREFIX . "fee_individual_plan sfp");
                            $this->db->join(DB_PREFIX . "course_mst cm", "sfp.CourseID=cm.CourseID", "left");
                            $this->db->join(DB_PREFIX . "fee_type_mst ftype", "sfp.FeeTypeID=ftype.FeeTypeID", "left");
              ...
              public function insert_scpf($FormData) {
                            ...
                            $trow = $FormData['total_row'];
                            for ($i = 0; $i < $trow; $i++) {
                                          if ($FormData['FeeTypeID'][$i] != "" && $FormData['Inst_amt'][$i] != "" && $FormData['Total_Inst'][$i] != "") {
              ...
              public function get_fee_records($filter_data) {
                            ...
                            if (isset($filter_data['amount_shared'])) {
                                          switch ($filter_data['amount_shared']) {
                                                        case "calculated":
                                                                      $this->db->where("amount_shared", TRUE);
                                                                      break;
                                                        case "not-calculated":
                                                                      $this->db->where("amount_shared", FALSE);
                                                                      break;
                            ...
                            $this->db->select('stu_mst.EnrollNo,'
                                    . 'stu_mst.StudentName,'
                                    . 'stu_mst.Mobile1,'
                            ...
                            $query = $this->db->get();
                            if(isset($filter_data['result_in'])){
              ...
              function fee_insert($FormData) {
                            ...
                            if ($FormData['Fee_Collect'] == "Save") {
                                          ...
                                          if ($FormData['Individual_fee_plan_id'] != 0) {
                                                        $sql = "update " . DB_PREFIX . "fee_individual_plan set Total_paid  = Total_paid+1 where Stu_ID={$FormData['Stu_ID']}  and ID = " . $FormData['Individual_fee_plan_id'];
                                                        $this->db->query($sql);
                                                        if (!$this->db->affected_rows()) {
                                                                      $this->db->trans_rollback();
...
//                                                         $this->db->trans_rollback();
//                                                        return array("succ" => FALSE, "_err_codes" => "ErrStuFacultyShareSavedErr" . ERR_DELIMETER);
//                                          }
                                          ...
                                          if ($this->db->trans_status() === TRUE) {
                                                        ...
                                                        $this->db->trans_rollback();
                                                        return array("succ" => FALSE, "_err_codes" => "ErrFeeSavedErr" . ERR_DELIMETER);
                                          }
                            }
                            return false;
              }
              ...
              public function fee_update() {
                            ...
                            if ($FormData['Paid_ModeID'] != 1) { // 1 represent cash   
                                          ...
                                          if (empty($cheque_details[0])) {
                                                        ...
                                                        if (!$this->db->update(DB_PREFIX . "fee_cheque_details", $cheque_data, array("ChequeID" => $cheque_details[0]->ChequeID))) {
                                                                      $this->db->trans_rollback();
              ...
              public function total_installments_amount($Stu_ID) {
                            $this->db->select_sum('PaidAmt')->where(array("Stu_ID" => $Stu_ID));
              ...
              function cancel_fee_receipt($FormData) {
                            ...
                            // cancel receipt
                            $data_to_update = array("Status" => FALSE);
                            $data_to_update = $this->util_model->add_mode_user($data_to_update);
                            $this->db->update(DB_PREFIX . "fee_trn", $data_to_update, array("ReceiptNo" => $FormData['ReceiptNo']));
                            if (!$this->db->affected_rows()) {
                                          $this->db->trans_rollback();
                            ...
                            $sql = "update " . DB_PREFIX . "fee_individual_plan set Total_paid = Total_paid-1 where Stu_ID={$FormData['ReceiptNo']}  and FeeTypeID={$FormData['FeeTypeID']}";
                            $this->db->query($sql);
                            ...
                            // deleting the faculty share
                            $this->db->where("ReceiptNo", $FormData['ReceiptNo']);
                            $this->db->delete(DB_PREFIX . "fee_faculty_account");
...
//        $sql = "update nexgen_scnb set Minimum_Total = Minimum_Total- $amount_to_less where EnrollNo='{$result[0]->EnrollNo}' and CourseCode='{$result[0]->CourseCode}'";
              ...
              public function get_no_of_installment($Stu_ID, $FeeTypeID = "") {
              ...
              public function last_fee_record($Stu_ID, $FeeTypeID = "") {
                            if ($FeeTypeID != "") {
                                          $this->db->where(array("FeeTypeID" => $FeeTypeID));
                            }

                            $this->db->order_by('ReceiptNo', 'DESC');
                            $query = $this->db->get_where(DB_PREFIX . "fee_trn", array("Stu_ID" => $Stu_ID));
                            $all_stu_scnb_details = $query->result();
                            return $all_stu_scnb_details;
              }
...
//        }
              ...
              function check_fully_paid($Stu_ID, $CourseID = '') {
                            ...
                            // getting balance
                            $bal_query = $this->db->get_where(DB_PREFIX . "fee_bal", array("Stu_ID" => $Stu_ID));
                            $bal_result = $bal_query->row_array();

                            // $this->util_model->printr($bal_result);
                            $Fully_paid = FALSE;
                            if (empty($result)) {
                                          $Fully_paid = TRUE;
                                          if (!empty($bal_result)) {
                                                        if ($bal_result['BalanceAmt'] != 0) {
                                                                      $Fully_paid = FALSE;
                                                        }
                                          }
                            }
               ...
               * funciton get_total_fees($Stu_ID)
               * return totalfees from facultyshare               */

              function get_total_fees($Stu_ID) {
                            $this->db->where(array("Stu_ID" => $Stu_ID, "Status" => TRUE));
                            $this->db->select_sum("DisFee")->from(DB_PREFIX . "fee_course_indvidual_share");
                            $result = $this->db->get()->result();
                            if ($result[0]->DisFee) {
                                          return $result[0]->DisFee;
                            } else {
                                          return 0;
                            }
              }
               ...
               * to get indivial_fee_plan total amount               */

              function get_total_fee_plan_amt($Stu_ID) {

                            $this->db->select(" sum(Inst_amt*Total_Inst) as total")->from(DB_PREFIX . "fee_individual_plan")->where("Stu_ID", $Stu_ID);
