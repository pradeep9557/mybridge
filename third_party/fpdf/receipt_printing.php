...
// Page header
    ...
    $pdf->SetY($pdf->GetY() - 3);
    $pdf->SetX((190 - $w) / 2 + 13);
    $pdf->Cell($w, 10, $Branch_obj->Add1);
    $pdf->SetY($pdf->GetY() + 3);
    $pdf->SetX((195 - $w) / 2 + 5);
    $pdf->Cell($w, 10, 'Website: '.$Branch_obj->Bsite);
    $pdf->SetY($pdf->GetY() + 4);
    $pdf->SetX((200 - $w) / 2 + 18);
    $pdf->SetTextColor(196, 100, 46);
    $pdf->Cell($w - 5, 10, 'E-mail:'.$Branch_obj->Email1);
...
//$pdf->Cell(15);
    if($page==1){
     $pdf->Cell(15, $pdf->GetY()-136, $Greating_Message);   
    }else
...
//1
    ...
    $pdf->Cell(36, 3, " :  " . $fee_record->ReceiptNo, 0, 0, 'L');
...
//3
    ...
    $pdf->MultiCell(125, 3, "Address         :".$fee_record->C_Add, 0, 'L');
...
// row 
    ...
    $pdf->MultiCell(180,3,"1. After Due Date() Of Every Month, "
            . "You will be liable to pay fine 5rs/- per Day.",0,'L');
    $pdf->MultiCell(180,3,"2. Your Registration will be cancelled, If you will be left or absent more than 15 days.",0,'L');
